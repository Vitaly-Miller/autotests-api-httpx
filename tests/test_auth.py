...
def test_login(function_user: UserFullSchema, auth_client: AuthClient):  # Передача фикстур СОЗДАНИЯ ПОЛЬЗОВАТЕЛЯ и авторизации: c аннотациями
    #----------------------------------------------------- Log in  -----------------------------------------------------
    # Инициализация модели с fake-данными нового пользователя по Pydantic-схеме
    login_payload = LoginRequestSchema(
        email=function_user.email,                                       # Email из .метода UserFullSchema
        password=function_user.password                                  # Password из .метода UserFullSchema
    )
