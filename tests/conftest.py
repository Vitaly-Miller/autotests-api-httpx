...
def create_user(public_users_client: PublicUsersClient) -> UserFullSchema:
    ...
    response = public_users_client.create_user(payload=create_user_payload)  # ︎▶ Запро-с на создание пользователя через метод. Передаем сгенерированные в Pydantic-схеме fake-данные нового пользователя
    return UserFullSchema(request=create_user_payload, response=response)    # Pydantic-model: UserFullSchema ✨с объединенными данными о пользователе <Request + Response>
...
"""---------------------------------------------- Auth (Authentication) ---------------------------------------------"""
#---------------------------------------------------- Pydantic-model ---------------------------------------------------
#--- v.1 --- Базируется на Pydantic-model: UserFullSchema - объединенной схеме <Response + Request>
@pytest.fixture
...
#--- v.2 --- Базируется на Request Body <response.request.content> (Парсинг)
# ...В разработке...
...
def auth_api_2(create_user_api: Response, auth_client: AuthClient) -> Response:
    ...
    :return: httpx.Response
    """
    request_body = json.loads(create_user_api.request.content)  # ✨Парсинг Request JSON —> {dict}
    login_payload = LoginRequestSchema(              # Инициализация модели с fake-данными нового пользователя по Pydantic-схеме
        email=request_body['email'],                 # Email из распарсенного Request
        password=request_body['password']            # Password распарсенного Request
    )
    response = auth_client.login_api(login_payload)  # ▶ Запрос на Login (Authentication) через API-метод. Передаем payload c Email и Password и сохраняем ответ в переменную
    return response                                  # httpx.Response
