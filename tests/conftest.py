...
def get_user_me(private_users_client: PrivateUsersClient) -> httpx.Response:  # Передаем фикстуру для Pre-condition
    get_user_me_client = private_users_client         # Получаем экземпляр PrivateUsersClient c уже настроенным HTTP-клиентом с Base URL и Авторизированным пользователем
