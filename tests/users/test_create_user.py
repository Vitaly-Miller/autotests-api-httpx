...
class TestCreateUser:
    ...
    def test_create_user_4_manual(self):
        ...
        # Assertions:
        # Base API assertions
        assert response.status_code == 200, '❌Incorrect status code!'           # проверка статус-кода    (⚠️Hard coding не рекомендуется)
        assert response.request.method == 'POST', '❌Incorrect Request Method!'  # проверка метода запроса (⚠️Hard coding не рекомендуется)
