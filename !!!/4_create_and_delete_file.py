...
login_response_data = login_response.json()         # Сохраняем JSON-ответ в переменную
token = login_response_data['token']['accessToken'] # Вытаскиваем по индексу [] токен из login_response_data

auth_headers = {'Authorization': f'Bearer {token}'} # Формируем авторизационный headers c токеном для дальнейших запросов c авторизацией
