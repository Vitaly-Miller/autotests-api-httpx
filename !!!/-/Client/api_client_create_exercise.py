...
create_user_response = public_users_client.create_user_pydantic(create_user_data=create_user_data)
...
create_file_response = files_client.create_file_pydantic(create_file_data=create_file_data)


#-------------------------------------------------- 3. Create Course  --------------------------------------------------
# Инициализация Pydantic-model с данными о курсе
create_course_pydantic_data = CreateCourseRequestSchema(      # Модель с данными о курсе
...
create_exercise_pydantic_data = CreateExerciseRequestSchema(
    courseId=create_course_pydantic_response.course.id
)
...
print(f'  Course ID: {create_course_pydantic_response.course.id}')
print(f'Exercise ID: {create_exercise_pydantic_response.exercise.id}')
